...
document.querySelector('#user-btn').onclick = () => {
   profile.classList.toggle('active'); // Basculer l'état actif du profil utilisateur
...
document.querySelector('#search-btn').onclick = () => {
   ...
   profile.classList.remove('active'); // Masquer le profil utilisateur si actif
}
...
// Ajouter un gestionnaire d'événements pour chaque input de type "number"
document.querySelectorAll('input[type="number"]').forEach(InputNumber => {
   InputNumber.oninput = () => {
      // Limiter la longueur de la valeur saisie à la longueur maximale définie
      if(InputNumber.value.length > InputNumber.maxLength) 
         InputNumber.value = InputNumber.value.slice(0, InputNumber.maxLength); // Tronquer la valeur si elle dépasse la longueur maximale
   }
});

// Ajouter un gestionnaire d'événements pour le défilement de la fenêtre
window.onscroll = () => {
   profile.classList.remove('active'); // Masquer le profil utilisateur
